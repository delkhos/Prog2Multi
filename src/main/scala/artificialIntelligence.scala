...
class SerpentIA extends ArtificialIntelligence(){ 
  ...
  override def processDecision(game: GameObject , monster: Monster){
    if(monster.state != State.Dead){ //test in case the monster is dead in this turn and still is in the memory
      //println(monster + " " + monster.pos.x + " " + monster.pos.y)
      if(game.lineOfSight(monster.pos,game.player.pos, game.current_floor)){ //determines if the monster can see the player
      ...
      if(monster.state == State.Idle){ //random movement while idling
        ...
        var i = r.nextInt(nextPositions.length)
        var delta = nextPositions(i)
        while( game.occupied(monster.pos.translate(delta._1,delta._2),game.current_floor)) //look for an unoccupied tile around the monster
        {
          nextPositions = nextPositions.take(i) ++ nextPositions.drop(i + 1)
          i = r.nextInt(nextPositions.length)
          delta = nextPositions(i)
        ...
        if (cooldown == 0){
          var attack_try = r.nextInt(100)
          if(attack_try <= 95){
            if (game.player.emerald){
              duration = 3
            }  
            game.player.status = (new Poison(new Sprite(Array[SubSprite](new SubSprite(299,SColor.BatPurple)), new Color((1.0).toFloat,(1.0).toFloat,(1.0).toFloat,(0.0).toFloat)),duration,game.player))::game.player.status          
